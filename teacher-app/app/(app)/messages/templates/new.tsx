// Create a new message template with live variable preview.
...
export default function NewTemplateScreen() {
  ...
  const [body, setBody] = useState(DEFAULT_BODIES['payment_reminder']);
  const [isDefault, setIsDefault] = useState(false);
  const [saving, setSaving] = useState(false);
  const [showPreview, setShowPreview] = useState(false);

  function onTypeChange(t: TemplateType) {
    setType(t);
    setBody(DEFAULT_BODIES[t]);
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await saveTemplate(teacherId, { type, language, body: body.trim(), isDefault });
      router.back();
    } catch {
...
function buildStyles(colors: ReturnType<typeof useThemeStore.getState>['colors']) {
  return StyleSheet.create({
    ...
    varChipText: { fontSize: 12, fontWeight: '600', fontVariant: ['tabular-nums'] },
